...
class FlowAssert<T> internal constructor(
  ...
  fun noEvents() {
    ...
    if (event != null) {
      throw AssertionError("Expected no events but found $event", event)
    }
  }
  ...
  suspend fun noMoreEvents() {
    ...
    if (event != null) {
      throw AssertionError("Expected no more events but found $event", event)
    }
  }
  ...
  suspend fun item(): T {
    ...
    if (event !is Event.Item<T>) {
      throw AssertionError("Expected item but was $event", event)
    }
    return event.item
  }
  ...
  suspend fun complete() {
    ...
    if (event != Event.Complete) {
      throw AssertionError("Expected complete but was $event", event)
    }
  }
  ...
  private fun <T> AssertionError(msg: String, event: Event<T>): AssertionError {
    return if (event is Event.Error) {
      AssertionError(msg, event.throwable)
    } else {
      AssertionError(msg)
    }
  }
